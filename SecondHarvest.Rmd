...
# Set directories for reading and saving files
data_dir <- "data"
maps_dir <- "maps"

# Cache tigris data and avoid re-downloading in future R sessions
options(tigris_use_cache = TRUE)
yr = 2021 # For US Census data and default year for Tigris

# Set census API key
if (is.null(Sys.getenv("CENSUS_API_KEY"))) {
  census_key_path <- file.path(dirname(getwd()), "inputs", "census_api_key.txt")
  census_api_key <- readr::read_file(census_key_path)
  census_api_key(census_api_key, install = TRUE)
}

# Set Google API key (for ggmap)
if (is.null(Sys.getenv("GOOGLE_API_KEY"))) {
  ggmap_key_path <- file.path(dirname(getwd()), "inputs", "ggmap_api_key.txt")
  ggmap_api_key <- readr::read_file(ggmap_key_path)
  register_google(ggmap_api_key, write = TRUE)
}

# Download partners list
if (!file.exists(file.path(data_dir, "SHFB_Partners.pdf"))) {
  # Download partners list pdf
  partner_url = "https://feedoc.org/wp-content/uploads/2024/08/SHFB-Partner-List-08-12-24.pdf"
  download.file(partner_url, destfile = file.path(data_dir, "SHFB_Partners.pdf"), mode = "wb")
}

# Download TEFAP list
if (!file.exists(file.path(data_dir, "TEFAP.pdf"))) {
  # Download partners list pdf
  tefap_url = "https://feedoc.org/wp-content/uploads/2024/08/SHFB-TEFAP-Distribtions-08-14-24.pdf"
  download.file(tefap_url, destfile = file.path(data_dir, "TEFAP.pdf"), mode = "wb")
}
# OC MEDIAN HOUSEHOLD INCOME 2021: 100,485
```
...
oc_cities <- st_read(dsn = file.path(data_dir, "orange_county.gpkg"), layer = "oc_cities", quiet = TRUE)
oc_tracts <- st_read(dsn = file.path(data_dir, "orange_county.gpkg"), layer = "oc_tracts", quiet = TRUE)
oc_blockgroups <- st_read(dsn = file.path(data_dir, "orange_county.gpkg"), layer = "oc_blockgroups", quiet = TRUE)
```
...
```{r, include = FALSE}
partners_sf <- st_read(file.path(data_dir, "partners.geojson"), quiet = TRUE)
```
...
```{r, get-acs, results='hide', message = FALSE}
...
```{r, blockgroups-buffers, results='hide', message = FALSE}
...
```{r, blockgroups-centroid-radius, results='hide', message = FALSE}
...
```{r}
oc_blockgroups %>% filter(CensusTract=="866.01")
```

```{r}
oc_tracts %>% filter(CensusTract=="866.01")
```
